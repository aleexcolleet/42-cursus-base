...
*	looks for complementary colors
*
*	color: original color (as integer in hexadecimal form)
*	percent : percentage by which the new color should be different
*		from the original color
*
*	function needs to treat colors separately
*	1. extracts the components from the provided color by bit-shifting and masking
*	2. calculates a new color component(trgb) (% of max value--> 256)
*	3. If this calculation results in a value exceeding 256, it wraps around by subtracting 256 from the result. This ensures that the resulting color component remains within the valid range (0-255).
*	4. Finally, it combines the modified RGB components into a single integer 
