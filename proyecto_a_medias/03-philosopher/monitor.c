...
void	*monitor_dinner(void *data)
{
	...
	while (!all_threads_running(&tmp_data->table_mutex,
							 &tmp_data->threads_running_nbr, tmp_data->num_philo, data))//TODO
		;
	while (!simulation_finished(tmp_data))
	{
		...
		{
			if (philo_died(tmp_data->philos + i, data)) //TODO
			{
				set_bool(&tmp_data->table_mutex, &tmp_data->end_simulation, true, data);
